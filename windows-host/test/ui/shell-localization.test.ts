...
function localizationFixture(): {
  ...
  const root = {
    ...
    querySelectorAll(selector: string) {
      if (selector === "[data-i18n]") return [navigation];
      if (selector === "[data-i18n-count]") return [freeRemaining];
      if (selector === "[data-i18n-aria-label]") return [sidebar];
      if (selector === "[data-i18n-route-title]") return [route];
      return [];
    },
  } as unknown as Document;
  return { root, navigation, freeRemaining, sidebar, route };
}

test("Operations Desk shell switches every binding type across all launch locales", () => {
  const fixture = localizationFixture();
  const expectations = [
    ["en-US", "Checks", "3 free real checks remain."],
    ["en-CA", "Cheques", "3 free real cheques remain."],
    ["fr-CA", "Chèques", "Il reste 3 vrais chèques gratuits."],
  ] as const;

  for (const [locale, navigation, freeRemaining] of expectations) {
